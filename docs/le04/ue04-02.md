...
??? success "Lösungsvorschlag"
    ...
    #Query-Resultat wird in qresult gespeichert
    qresult = cur.execute(query, (f"{band}%",))    

    #Das Fetchall geht ins fetchresult-Tupel. Wenn dieses leer ist, wurde kein Resultat gefunden.
    fetchresult = qresult.fetchall()    

    for row in fetchresult:
        print(row[0],row[1])    

    # check, ob Tupel fetchresult leer ist, dann Meldung
    if not fetchresult:
