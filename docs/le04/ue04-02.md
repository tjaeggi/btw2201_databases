...
??? success "Lösungsvorschlag"
    ...
    # eingeben würden.
    # Das Konstrukt f"{variable}%"} kreiert den Wert Inhalt der Variable+Prozentzeichen. Python-Thema: Formatierung von Ausdrücken in String-Literals.
    # ... und das ganze als Single-Element-Tupel (mit Komma)   
    # Siehe dazu: https://docs.python.org/3/library/sqlite3.html#how-to-use-placeholders-to-bind-values-in-sql-queries    

    #qresult ist immer vom typ <class 'sqlite3.Cursor'>.
    qresult = cur.execute(query, (f"{band}%",))    

    #Das Resultat von fetchall() geht in die fetchresult-Liste. Typ ist <class 'list'>. Wenn dieses leer ist, d.h.[], #wurde kein Resultat gefunden.
    fetchresult = qresult.fetchall()    

    for row in fetchresult:
        print(row[0],row[1])    

    # check, ob die Liste fetchresult leer ist ([]), dann Meldung "Kein Resultat"
    if not fetchresult:
