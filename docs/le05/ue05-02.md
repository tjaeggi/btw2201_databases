...
!!! example "UE05-02-Datentransfer von SQLite-DB nach MySQL mit Python und der sqlalchemy-Library"
    ...
    Laden Sie die Daten in die  Daten der Tabellen `new_PLZ1` und `new_STR` aus der SQLite-Datenbank in die MySQL-Datenbank.
...
??? success "Lösungsvorschlag"
    ...
    df_new_plz1.to_sql('NEW_PLZ1', con=con_mysql, if_exists='replace', index=False)
