...
!!! info "Hinweis"
    ...
    Siehe auch die markierten Code-Zeilen im Lösungsvorschlag und beachten Sie diese elegante und einfache Möglichkeit, mit `Pandas` und `sqlalchemy` Tabellen mit Daten in einer Datenbank zu kreieren.

    `sqlalchemy` ünterstützt die Datenbanken `SQLite`, `Postgresql`, `MySQL` & `MariaDB`, `Oracle`, and `MS-SQL`. 

    <figure markdown="span">
    ![Image title](../img/alchemicdatabaseenvironment.png){ width="400" }
    <figcaption>using SQLAlchemy-Library</figcaption>
    </figure>
...
??? success "Lösungsvorschlag"
    ...
    Needed Libraries mit pi: python3 -m pip install ....
    pip install PyMySQL pandas sqlalchemy mysql-connector-python
    oder wenn Sie mit anaconda als Library-Manager arbeiten:
    ...
    from sqlalchemy.sql import text
    # needed for wrapping SQL-statements into text()    
    ...
    result1 = con_mysql.execute(text("SELECT * FROM NEW_STR LIMIT 5"))
    for row in result1:
        print(row)
        
    # Is there data in Table NEW_PLZ1 ?
    result3 = con_mysql.execute(text("SELECT * FROM NEW_PLZ1 LIMIT 5"))
    for row in result3:
        print(row)
        
    # How many rows in Table NEW_PLZ1 ?
    result4 = con_mysql.execute(text("SELECT count(*) FROM NEW_PLZ1"))
    for row in result4:
        print("rows transfered into NEW_PLZ1:",row[0])    

    # How many rows in Table NEW_STR ?
    result2 = con_mysql.execute(text("SELECT count(*) FROM NEW_STR"))
    for row in result2:
